...
STATUS = {
    "Uploading":CaptureStatus(latest_run_current_stage="Uploading", latest_run_status="New"),
    "Queued":CaptureStatus(latest_run_current_stage="Queued", latest_run_status="New"),
...
def delete_a_capture(db:Session, uuid:str)->dict:
    db_capture = db.query(Capture).filter(Capture.uuid == uuid).first()
    if db_capture is not None:
        ...
        raise Exception(f"{uuid} Capture not found")
